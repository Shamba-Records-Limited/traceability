...
//
// Shape mirrors services/hedera-publisher/internal/config so operators can
// share environment files across both services. When a shared Hedera-client
// module exists, this file is the first thing to delete.
package config
...
// Config holds the resolved runtime configuration for the issuer service.
type Config struct {
	Network            Network
	OperatorID         string
...
func Load() (*Config, error) {
	cfg := &Config{
		...
		HTTPPort:           envDefault("HTTP_PORT", "8081"),
